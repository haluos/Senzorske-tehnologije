...
//convert float data to 4 byte char array, index 0 of array is least significant byte
	...
	time_char[3] = (uint8_t)(((uint32_t)tim & 0xFF000000) >> 24);
	time_char[2] = (uint8_t)(((uint32_t)tim & 0xFF0000) >> 16);
	time_char[1] = (uint8_t)(((uint32_t)tim & 0xFF00) >> 8);
	time_char[0] = (uint8_t)((uint32_t)tim & 0xFF);
	// time_char[0] = 0x11;
	// time_char[1] = 0x00;
	// time_char[2] = 0x00;
	// time_char[3] = 0x00;
	// memcpy(temp_char, &temp, sizeof(float));
	temp_char[3] = 0x00; temp_char[2] = 0x00;
	temp_char[1] = (uint8_t)((temp & 0xFF00) >> 8); temp_char[0] = (uint8_t)(temp & 0xFF);
